 ...
 *
 * @method  enable
 * @param   {boolean}  exists  True if sequence already exists, otherwise False
 */
Sequence.prototype.enable = function (exists) {
    this._counterModel = this._createCounterModel(exists);
 ...
 */
Sequence.prototype._createCounterModel = function (exists) {
    const schemaName = 'Counter_' + this._options.id;
    if (exists) return mongoose.model(schemaName);
    ...
    return mongoose.model(schemaName, CounterSchema);
};
